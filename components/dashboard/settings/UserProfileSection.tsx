...
export default function UserProfileSection() {
   ...
   const hasChanges = firstName !== (userProfile?.firstName ?? '') || lastName !== (userProfile?.lastName ?? '');
   ...
   return (
      <SectionCard id="profile">
         <SectionHeading icon={User} title="Your Profile" blurb="How you appear to teammates and in notifications." />

         {/* Avatar + identity row */}
         <div className="flex flex-col sm:flex-row sm:items-center gap-5 mb-6 pb-6 border-b border-outline-variant/10">
            <div className="relative shrink-0">
               <div className="w-20 h-20 rounded-full bg-primary-container/30 border-2 border-primary-container/50 flex items-center justify-center text-primary text-xl font-bold">{initials}</div>
               {/* <button
                  type="button"
            ...
            <div className="flex-1 min-w-0">
               <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-base font-semibold text-on-surface">{fullName}</h3>
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold tracking-wider uppercase bg-primary/10 text-primary">Owner</span>
               </div>
               <p className="text-xs text-on-surface-variant mt-1">{userProfile?.email}</p>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <Field hint="Used for sign-in and notifications." label="Email" value={userProfile?.email} disabled />
         </div>
